...
c.execute('SELECT question_text, correct_answer, choice_1, choice_2, choice_3, choice_4 FROM questions_m WHERE difficulty = 1')
easy_questions_data = c.fetchall()

c.execute('SELECT question_text, correct_answer, choice_1, choice_2, choice_3, choice_4 FROM questions_m WHERE difficulty = 2')
medium_questions_data = c.fetchall()

c.execute('SELECT question_text, correct_answer, choice_1, choice_2, choice_3, choice_4 FROM questions_m WHERE difficulty = 3')
...
for i, (_, _, choice_1, choice_2, choice_3, choice_4) in enumerate(questions_data):
    button_1 = Button(pygame.Rect(180, 350, 100, 90), choice_1)
    button_2 = Button(pygame.Rect(370, 350, 100, 90), choice_2)
    button_3 = Button(pygame.Rect(180, 470, 100, 90), choice_3)
    button_4 = Button(pygame.Rect(370, 470, 100, 90), choice_4)
...
def display_instructions():
    ...
    while True:
        ...
        # Draws the instructions rectangle
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN and restart_button is not None and restart_button.rect.collidepoint(event.pos):
            ...
            scoreboard.insert_math_score(conn, scoreboard.update_score(), total_time, user_id)
            reset_game_state()
    ...
    if not questions:
        questions = [
            Question(pygame.Rect(80, question_y + i * 150, 500, 400), question_text, correct_answer)
